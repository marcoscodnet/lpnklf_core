...
class LpnklfUtils {
	...
	const LPNKLF_CONCEPTO_GASTO_VARIOS = 1;
	
	const LPNKLF_CONCEPTO_MOVIMIENTO_GASTO= 2;
	const LPNKLF_CONCEPTO_MOVIMIENTO_ANULACIONGASTO= 3;
	const LPNKLF_CONCEPTO_MOVIMIENTO_VENTA= 4;
	const LPNKLF_CONCEPTO_MOVIMIENTO_ANULACIONVENTA= 5;
	const LPNKLF_CONCEPTO_MOVIMIENTO_PAGO= 6;
	const LPNKLF_CONCEPTO_MOVIMIENTO_PEDIDO= 7;
	const LPNKLF_CONCEPTO_MOVIMIENTO_ANULACIONPEDIDO= 8;
	const LPNKLF_CONCEPTO_MOVIMIENTO_SERVICIOTECNICO= 9;
	const LPNKLF_CONCEPTO_MOVIMIENTO_ANULACIONSERVICIOTECNICO= 10;
	
	const LPNKLF_CUENTA_UNICA = 1;
	
	const LPNKLF_CAJA_DEFAULT = 1;
	
	const LPNKLF_ID_DOLAR = 1;
	const LPNKLF_ID_PORCENTAJE_PRECIO_LISTA = 2;
	
	const LPNKLF_TIPO_PRODUCTO_DEFAULT = 21;
	const LPNKLF_MARCA_PRODUCTO_DEFAULT = 24;
    ...
    public static function sendMailPop($nameFrom, $from, $nameTo, $to, $subject, $msg, $attach, $CustomHeaderArray) {
		$libsHome= NewsletterConfig::getInstance()->getLibsPath();
       ...
       // ob_start();
		...
		$mail->Host = NewsletterConfig::MAIL_HOST;
		//Set the SMTP port number - likely to be 25, 465 or 587
		$mail->Port = NewsletterConfig::MAIL_PORT;
		$mail->SMTPSecure = NewsletterConfig::MAIL_SMTP_SECURE;
		//Whether to use SMTP authentication
		$mail->SMTPAuth = true;
		//Username to use for SMTP authentication
		$mail->Username = NewsletterConfig::MAIL_USERNAME;
		//Password to use for SMTP authentication
		$mail->Password = NewsletterConfig::MAIL_PASSWORD;
        ...
        if (!$mail->Send()){
            ...
            throw new ServiceException("Ocurrió un error en el envío del mail a $nameTo <$to>");
        }
    ...
    public static function sendMail($nameFrom, $from, $nameTo, $to, $subject, $msg, $attach='', $CustomHeaderArray=array()) {
        ...
        if (NewsletterConfig::MAIL_ENVIO_POP)
            ...
            if (!mail($to2, $subject, $msg, $headers)){
            	...
            	throw new ServiceException("Ocurrió un error en el envío del mail a $to2");
            }
            //para que no de la salida del mailer.
            ob_end_clean();
        }
	...
	public static function getConceptoGastoVarios(){
		...
		return ServiceFactory::getConceptoGastoService()->get( self::LPNKLF_CONCEPTO_GASTO_VARIOS ); 
	}
	
	public static function getConceptoMovimientoGasto(){
	
		return ServiceFactory::getConceptoMovimientoService()->get( self::LPNKLF_CONCEPTO_MOVIMIENTO_GASTO ); 
	}
	
	public static function getConceptoMovimientoAnulacionGasto(){
	
		return ServiceFactory::getConceptoMovimientoService()->get( self::LPNKLF_CONCEPTO_MOVIMIENTO_ANULACIONGASTO ); 
	}
	
	public static function getConceptoMovimientoVenta(){
	
		return ServiceFactory::getConceptoMovimientoService()->get( self::LPNKLF_CONCEPTO_MOVIMIENTO_VENTA ); 
	}
	
	public static function getConceptoMovimientoAnulacionVenta(){
	
		return ServiceFactory::getConceptoMovimientoService()->get( self::LPNKLF_CONCEPTO_MOVIMIENTO_ANULACIONVENTA ); 
	}
	
	public static function getConceptoMovimientoPago(){
	
		return ServiceFactory::getConceptoMovimientoService()->get( self::LPNKLF_CONCEPTO_MOVIMIENTO_PAGO); 
	}
	
	public static function getConceptoMovimientoServicioTecnico(){
	
		return ServiceFactory::getConceptoMovimientoService()->get( self::LPNKLF_CONCEPTO_MOVIMIENTO_SERVICIOTECNICO ); 
	}
	
	public static function getConceptoMovimientoAnulacionServicioTecnico(){
	
		return ServiceFactory::getConceptoMovimientoService()->get( self::LPNKLF_CONCEPTO_MOVIMIENTO_ANULACIONSERVICIOTECNICO ); 
	}
	
	public static function getCuentaUnica(){
	
		return ServiceFactory::getCuentaService()->get( self::LPNKLF_CUENTA_UNICA ); 
	}
	
	public static function getCajaDeafault(){
	
		return ServiceFactory::getCajaService()->get( self::LPNKLF_CAJA_DEFAULT ); 
	}
	
	public static function getPrecioDolar(){
	
		return ServiceFactory::getParametroService()->get( self::LPNKLF_ID_DOLAR ); 
	}
	
	public static function getPorcentajePrecioLista(){
	
		return ServiceFactory::getParametroService()->get( self::LPNKLF_ID_PORCENTAJE_PRECIO_LISTA ); 
	}
	
	public static function getTipoProductoDefault(){
	
		return ServiceFactory::getTipoProductoService()->get( self::LPNKLF_TIPO_PRODUCTO_DEFAULT ); 
	}
	
	public static function getMarcaProductoDefault(){
	
		return ServiceFactory::getMarcaProductoService()->get( self::LPNKLF_MARCA_PRODUCTO_DEFAULT ); 
	}
